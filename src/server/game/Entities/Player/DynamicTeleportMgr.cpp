...
bool DynamicTeleportMgr::Init()
{
    ...
    if(!result)
    {
        sLog->outInfo(LOG_FILTER_GENERAL, ">> Loaded 0 Dynamic Teleporter rows. DB table `dynamic_teleporter` is empty!");
        return false;
    }
    ...
    sLog->outInfo(LOG_FILTER_GENERAL, ">> Loaded %u Dynamic Teleporter rows in %u ms", counter, GetMSTimeDiffToNow(oldMSTime));
